...
# Life Ball Constants
LIFE_BALL_BLINK_SPEED = 0.5  # Speed of life ball blinking
life_ball_active = False     # Whether life ball is currently in the game
life_ball_blink_phase = 0    # Current phase of life ball blinking
life_ball_spawn_level = 2    # Level at which life ball starts appearing
life_ball_spawn_interval = 2 # Spawn every 2 levels

# Star effect constants
STAR_MOVEMENT_FACTOR = 0.5  # How much stars move relative to ball movement
STAR_BLINK_SPEED = 0.5     # Speed of star blinking
star_offset_x = 0          # Current star offset for parallax effect
star_blink_phase = 0       # Current phase of star blinking
...
# Balls
class Ball:
    def __init__(self, x, y, z, radius, color, is_life_ball=False):
        self.x = x
        self.y = y
        self.z = z
        self.radius = radius
        self.color = color
        self.is_life_ball = is_life_ball
        self.blink_phase = 0

    def draw(self):
        glPushMatrix()
        if self.is_life_ball:
            # Calculate brightness based on blink phase
            brightness = 0.5 + 0.5 * math.sin(self.blink_phase)
            glColor3f(brightness, brightness, brightness)
            # Update blink phase
            self.blink_phase = (self.blink_phase + LIFE_BALL_BLINK_SPEED) % 360
        else:
            glColor3f(self.color[0], self.color[1], self.color[2])
        glTranslatef(self.x, self.y, self.z)
...
def update_star_effects():
    """Update star positions and blinking based on ball movement"""
    global star_offset_x, star_blink_phase, direction, main_ball_pos
    
    # Update star offset based on ball movement
    if direction == "LEFT":
        star_offset_x = min(star_offset_x + STAR_MOVEMENT_FACTOR, 20)
    elif direction == "RIGHT":
        star_offset_x = max(star_offset_x - STAR_MOVEMENT_FACTOR, -20)
    else:
        # Gradually return stars to center when ball is not moving
        if star_offset_x > 0:
            star_offset_x = max(0, star_offset_x - STAR_MOVEMENT_FACTOR)
        elif star_offset_x < 0:
            star_offset_x = min(0, star_offset_x + STAR_MOVEMENT_FACTOR)
    
    # Update blinking phase
    star_blink_phase = (star_blink_phase + STAR_BLINK_SPEED) % 360

def draw_stars():
    """Draw stars in the background with parallax and blinking effects"""
    glPushMatrix()
    
    # Apply parallax offset
    glTranslatef(star_offset_x, 0, 0)
    
    # Draw stars with varying brightness based on blinking phase
    for x, y in stars_positions:
        # Calculate brightness based on position and phase
        brightness = 0.5 + 0.5 * math.sin(star_blink_phase + x * 10)
        glColor3f(brightness, brightness, brightness)
        
        # Draw star with varying size based on brightness
        glPointSize(2 + brightness)
        glBegin(GL_POINTS)
...
def reset_game():
    ...
    global speed_multiplier, game_speed, base_game_speed, life_ball_active
...
def spawn_life_ball():
    """Spawn a new life ball"""
    global life_ball_active, balls
    
    if not life_ball_active and level >= life_ball_spawn_level and (level - life_ball_spawn_level) % life_ball_spawn_interval == 0:
        # Random x position (-80, 0, or 80)
        x_pos = random.choice([-80, 0, 80])
        # Create new life ball
        new_ball = Ball(x_pos, 0, -600, 20, [1.0, 1.0, 1.0], True)
...
def update_game_state():
    ...
    global main_ball_radius, size_platforms, life_ball_active
    ...
    # Check if we should spawn a life ball
    spawn_life_ball()
    ...
    for ball in balls:
        ...
        if abs(ball.x - main_ball_pos[0]) < 40 and \
           ...
           abs(ball.y - main_ball_y) < 40:
            ...
            if ball.is_life_ball:
                # Handle life ball collision
                if lives < 3:  # Only add life if not at max
                    lives += 1
                life_ball_active = False
                ball.x = random.choice([-80, 0, 80])
                ball.z = -600
                ball.color = random.choice(colors)
                ball.is_life_ball = False
            elif ball.color == main_ball_color:
